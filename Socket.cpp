...
void Socket::sendFile(const std::shared_ptr<SyncedFile>& syncedFile) {
    char buffer[N];
...
//    syncedFile->update_file_data();
    ...
    // todo: posso implementare una risposta del server, se il file è già presente non devo ricaricare il file
    ...
    if(syncedFile->isFile()) {
        ...
        while (size_read < file_to_send.tellg()) {
